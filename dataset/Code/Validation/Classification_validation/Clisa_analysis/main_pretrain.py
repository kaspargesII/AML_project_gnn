...
if label_type == 'cls2':
    args.log_dir = 'raw_24video_batch%d_dataset_%s_timeLen%d_tf%d_sf%d_multiFact%d_lr%f_wd%f_epochs%d_randSeed%d_fold%d_accSel_newPre_%s' % (
...
if pretrain: 
    ...
    for fold in folds_list:
        ...
        model_pre = ConvNet_baseNonlinearHead(n_spatialFilters, n_timeFilters, timeFilterLen, n_channs, stratified=stratified, multiFact=multiFact,
        ...
        if sample_method == 'cross':
            ...
            train_sampler = TrainSampler_video(len(train_sub), n_times=args.n_times, batch_size=args.batch_size_pretrain,
                                                    n_samples=n_samples)
